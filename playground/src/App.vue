...
<template>
  <div
    class="w-100vw h-100vh flex flex-col items-center place-content-center bg-gradient-to-r from-sky-400 to-blue-600">
    <button class="rounded-md bg-black/20 px-4 py-2 text-sm font-medium text-white
      hover:bg-black/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 cursor-pointer"
      @click="isOpen = true">
      支付
    </button>
    <Dialog :open="isOpen" class="fixed inset-0 bg-black/25 flex items-center justify-center">
      <DialogPanel class="absolute z-10 max-w-md transform overflow-hidden rounded-2xl bg-white p-6
        ...
        <button class="mt-4 inline-flex justify-center rounded-md border border-transparent
